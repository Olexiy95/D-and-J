...
public class Dungeon {
    ...
    public void start() {
        ...
        if (start == 'y') {
            ...
            do {
                try {
                    System.out.println(
                            "Welcome " + tempName + "! " + "Please select the difficulty: Easy, Moderate or Hard...");
            ...
            while (tiles > 0 && tempHealth > 0) {
                System.out
                        .println("Would you like to roll the dice? Type in yes to roll or anything else to exit game.");
                ...
                if (choice == 'y') {
                    ...
                    else if ((event >= 3) && (event <= 6) && (tempMonsterCount > 0) && (tiles > 0)) // monster
                    {
                        ...
                        System.out.println(Monster.display(tempMonsterType, tempMonsterHealth, tempMonsterMinHit,
                                tempMonsterMaxHit));
            ...
            if ((tiles <= 0) && (tempHealth > 0)) {
                ...
                try {
                    FileWriter write = new FileWriter("/home/Outcome.txt", true);
                    String score = tempName + " lost with " + tempStartPotions + " potions and " + tempCoins
                            + " coins.";
                    write.append(score);
                    score += "\n";
                    write.close();
    ...
    public static String progress(int tiles, int monsterCount, int coinCount, int potionCount) {
        return tiles + " tiles left, " + monsterCount + " monsters left. " + coinCount + " coins and " + potionCount
                + " potions in inventory.";
    }

    public static String win(String name, int potionCount, int coinCount) {
        return "Congratulations " + name + " you completed the dungeon! You finished with " + potionCount
                + " potions and " + coinCount + " gold coins!";

    }

    public static String loss(String name, int potionCount, int coinCount) {
        return "Better luck next time " + name + " you did not complete the dungeon! You finished with " + potionCount
                + " potions and " + coinCount + " gold coins.";
